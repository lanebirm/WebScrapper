...
class App:
    def __init__(self, email= "lanebirmbetnotify@gmail.com", password= "ruobyam1!", 
        ...
        self.scrape_item_details(self.used_item_links)
        self.driver.quit()
    ...
    def scrape_item_links(self):
        ...
        marketplace_button = self.driver.find_element_by_xpath("//*[@href='https://www.facebook.com/marketplace/?ref=bookmark']")
        marketplace_button.click()
        sleep(0.5)

        #create a list of each section and loop through list
        sections = [self.driver.find_element_by_xpath("//a[contains(@href, '/electronics/')]")]
        ...
        for category in sections:
            ...
            full_items_list = self.driver.find_elements_by_xpath("//a[contains(@href, '/marketplace/item/')]")
            if len(self.used_item_links) == 0: 
                ...
                for item in full_items_list:
                    try:
                        self.used_item_links = self.used_item_links.extend([item.get_attribute('href')])
                    except:
                        continue
